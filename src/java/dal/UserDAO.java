...
public class UserDAO extends BaseDAO {
    ...
    public List<UserDTO> getAllUserByGroup(int groupID) {
        ...
        String sql = "select u.*,c.isLeader,c.class_id from user u join class_user c on u.user_id = c.user_id where c.group = ?";
        try {
            Connection connection = getJDBCConnection();
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setInt(1, groupID);
            ...
            while (rs.next()) {
                ...
                udto.setUser(user);
                udto.setIsLeader(rs.getBoolean("isLeader"));
                udto.setCls(new ClassDAO().getClassByID(rs.getInt("class_id")));
                list.add(udto);
            }
        } catch (SQLException e) {
    ...
    public List<UserDTO> getStudentNoGroup() {
        List<UserDTO> list = new ArrayList<>();
        String sql = "select u.*,c.isLeader,c.class_id from user u join class_user c on u.user_id = c.user_id where c.group is null";
        try {
            Connection connection = getJDBCConnection();
            PreparedStatement ps = connection.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
